...
#include "cgilib.h"
#include "webget.h"
#define ENV_INLINE_HELP ENV_PREFIX "E"

/// A class to output help for individual options
class Helper {
public:
Helper() : theHelpFile(NULL) {};
~Helper();
void help(const char* option,CGIEnvironment* theEnvironment);
static void outputHelpSwitch(CGIEnvironment* theEnvironment);
static void dealWithHelpSwitch(CGIEnvironment* theEnvironment);
static Helper* hackyGlobalHelper() {
static Helper* h=NULL;
if(!h) h=new Helper;
return h;
// and who exactly deletes it? (probably doesn't matter
// in this application, but...)
}
private:
FILE* theHelpFile;
};

/// Outputs all the help
